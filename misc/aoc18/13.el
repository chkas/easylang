...
func run . .
  ...
  repeat
    # sort car pos
    for i = 0 to len car_pos[] - 2
      for j = i + 1 to len car_pos[] - 1
      ...
      .
    .
    for id range len car_pos[]
      pos = car_pos[id]
      if pos <> -1
        if f[pos] = 3
          car_dir[id] = 3 - car_dir[id]
        elif f[pos] = 4
          car_dir[id] = (1 - car_dir[id]) mod 4
        elif f[pos] = 5
          car_dir[id] = (car_dir[id] + car_turn[id] - 1) mod 4
          car_turn[id] = (car_turn[id] + 1) mod 3
        .
        posn = pos + dir[car_dir[id]]
        car_pos[id] = posn
        for j range len car_pos[]
