...
/*
void free_arr_members(struct arr* a, int i0) {
	if (a->typ == ARR_STR) {
...
static struct str op_substr(struct op* op) {
	struct str s = strf(op->o1);
	struct str r = str_substr(&s, (int)numf(op->o2), (int)numf(op->o3));
	str_free(&s);
	return r;
}
...
static void op_swapnumael(struct op* op) {
	struct arr* arr = garr(op->o1);
	int h1 = (int)numf(op->o2);
	int h2 = (int)numf(op->o3);
...
static void op_swaparraelx(struct op* op) {
	...
	struct arr* arr1 = arr->parr + h;
	h = (int)numf(op->o3);
	if (h < 0 || h >= arr->len) {
		out_of_bounds(op);
		return;
	}
	struct arr* arr2 = arr->parr + h;
	struct arr tmp = *arr1;
	*arr1 = *arr2;
	*arr2 = tmp;
}
...
static struct arr op_vnumarrarr(struct op* op) {
	struct arr* arr = garr(op->o1);
	struct arr res;
	res.len = arr->len;
	res.p = malloc(res.len * sizeof(struct arr));
	res.typ = ARR_ARR;
	if (res.p == NULL) {
		out_of_mem(op);
		res.len = 0;
	...
	for (int i = 0; i < res.len; i++) {
		...
		struct arr r;
		r.len = a->len;
		r.p = malloc(r.len * sizeof(struct arr));
		r.typ = ARR_NUM;
		if (r.p == NULL) {
			out_of_mem(op);
			r.len = 0;
...
static struct arr op_numarrarr_init(struct op* op) {
...
static void op_strarr_ass(struct op* op) {
	struct arr h = arrf(op->o2);
	struct arr* dest = garr(op->o1);
	if (dest->pstr != NULL) {
		for (int i = 0; i < dest->len; i++) {
			str_free(&dest->pstr[i]);
		}
		free(dest->pstr);
	}
	*dest = h;
...
static struct arr op_map_number(struct op* op) {
	...
	res.len = arr.len;
	res.typ = ARR_NUM;
	res.p = malloc(res.len * sizeof(double));
	if (res.p == NULL) {
		out_of_mem(op);
		res.len = 0;
		return res;
