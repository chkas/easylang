...
void gr_rect(double x, double y) {
	SDL_Rect r = {.x = gx, .y = gy, .w = (int)(FX * x) + 1, .h = (int)(FX * y) + 1}; 
...
static void handle_event(SDL_Event* evt) {
	switch (evt->type) {
		case SDL_MOUSEBUTTONDOWN:
			evt_mouse(0, evt->button.x / FMX, evt->button.y / FMY);
			break;
		case SDL_MOUSEBUTTONUP:
			evt_mouse(1, evt->button.x / FMX, evt->button.y / FMY);
			break;
		case SDL_MOUSEMOTION:
			evt_mouse(2, evt->motion.x / FMX, evt->motion.y / FMY);
			break;
		case SDL_KEYDOWN: {
