...
function easyrun(prog, ca = null, out = null, args = null, caret = -1) {
	eProg.push([prog, ca, out, args, caret])
	if (initState == 0) {
		easyinit(eProg[0][1])
	}
...
function tryrun() {
	...
	if (eProg.length != 0) {
		...
		if (eOut) eOut.value = ""
		isRunning = true
		worker.postMessage(["run", h[0], 2, h[4], h[3]])
	}
}
...
function workerMessage(event) {
	...
	case "ide":
		d.shift()
		var msg = d.shift()
		if (d[0] == "src_err") isRunning = false
		msgFunc(msg, d)
		break
	case "stderr":
		console.log(d[1])
