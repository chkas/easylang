...
static void cs_tok_nt(void) {
	cs(tval);
	nexttok();
}
static void cs_tok_spc_nt(void) {
	cs(tval);
	cs_spc();
	nexttok();
}

static void csb_tok_spc_nt(void) {
	csb(tval);
	cs_spc();
	nexttok();
}
static void csk_tok_spc_nt(void) {
	if (*tval >= 'a') csb(tval);
	else cs(tval);	
	cs_spc();
	nexttok();
...
static ushort parse_strex(void);
static ushort parse_strterm(void);
...
static ushort parse_numarrex(void);
static ushort parse_strarrex(void);

struct str (*strf[])(struct op*) = {
	op_input, op_sysfunc, op_keyb_key, op_str_chr, op_time_str, op_join, op_substr
};

static void parse_strfunc(ushort o) {
...
static ushort parse_log_ex(void);

static ushort parse_lenfunc(ushort mode) {
...
static ushort parse_termx(ushort ox) {
	...
	while (tok == t_mult || tok == t_div || (tok >= t_mod && tok <= t_divi1)) {
		...
		if (cod) {
			...
			else if (tok == t_mod) codp[o].numf = op_mod;
			else if (tok == t_divi1) codp[o].numf = op_divi1;
			else codp[o].numf = op_mod1;
		}
		csk_tok_spc_nt();
		h = parse_fac();
		codp[o].o2 = h;
...
static ushort parse_term(void) {
	return parse_termx(UMO);
}

static ushort optimize_ex(ushort ox, ushort o, ushort h) {
...
static void parse_arr_cmp(ushort o) {
	ushort h;
	h = parse_numarrex();
	codp[o].o1 = h;
	cs_spc();
	if (tok == t_eq) codp[o].intf = op_eqarr;
	else if (tok == t_neq) codp[o].intf = op_neqarr;
	else error("=, <>");
	cs_tok_spc_nt();
	h = parse_numarrex();
	codp[o].o2 = h;
...
static ushort parse_log_term(void) {
...
static ushort parse_log_ex_and(void) {
...
static void stat_begin_nest(void) {
...
// because p can be relocated don't:  p->id = h
...
static void parse_on_stat(void) {
	stat_begin_nest();
	int id = 0;
	ushort* psq;
...
static void parse_sys_stat(ushort o) {
	csb_tok_spc_nt();
...
static ushort parse_numarrarrex(void) {
...
static ushort parse_strarrarrex(void) {
...
//			h = parse_numarrex();
		...
		codp[ex].arrf = op_varrarr;
		codp[ex].o1 = parse_var(VAR_STRARRARR, RD);
		expt_ntok(t_brr);
	}
...
static void parse_arrbase_stat(ushort o) {
	parse_len_stat(o);
	if (codp[o].vf == op_arrnumarrel_len || codp[o].vf == op_arrstrarrel_len)
		codp[o].vf = op_arrbase2;
	else codp[o].vf = op_arrbase;
}
...
static ushort parse_strarr_term(void) {
	...
	while (1) {
		...
		if (tok == t_hash) {
			...
			if (tok == t_global) {
				...
				if  (sequ_level != 0) goto statement; // subr inside function
				parse_subr();
			}
			else if (tok == t_on) {
				if (sequ_level != 0) error("not allowed here");
